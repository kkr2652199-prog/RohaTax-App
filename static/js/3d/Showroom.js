...
class Showroom {
  constructor(containerId) {
    ...
    // 시네마 모드 플래그 (별도 모듈에서 제어)
    this.isCinemaMode = false;
    
    // 시네마 모드 컨트롤러 초기화
    console.log("🛠️ [Showroom Init] CinemaController 클래스 확인:", typeof window.CinemaController);
    if (typeof window.CinemaController !== "undefined") {
      this.cinemaController = new window.CinemaController(this);
      console.log("✅ [Showroom] CinemaController 초기화 완료");
      console.log("🛠️ [Showroom Init] Controller Status:", this.cinemaController);
      console.log("🛠️ [Showroom Init] Controller isActive:", this.cinemaController ? this.cinemaController.isActive : "N/A");
    } else {
      console.warn("⚠️ [Showroom] CinemaController를 찾을 수 없습니다. 시네마 모드가 비활성화됩니다.");
    ...
    // [Rescue] 컨트롤러가 생성자에서 실패했다면, 지금 다시 시도한다.
    if (!this.cinemaController && typeof window.CinemaController !== "undefined") {
      console.log("🚑 [Showroom] CinemaController 뒤늦게 발견! 연결합니다.");
      try {
        this.cinemaController = new window.CinemaController(this);
        console.log("✅ [Showroom] CinemaController 재연결 성공!");
      } catch (error) {
        console.error("❌ [Showroom] CinemaController 재연결 실패:", error);
        this.cinemaController = null;
      }
    } else if (!this.cinemaController) {
      console.warn("⚠️ [Showroom] CinemaController를 여전히 찾을 수 없습니다.");
      console.warn("   - window.CinemaController:", typeof window.CinemaController);
    }
    ...
    window.debugShowroom = this;
    console.log("🛠️ [Showroom] 인스턴스 전역 노출 완료: window.debugShowroom");
    console.log("🛠️ [Showroom] CinemaController 상태:", this.cinemaController);
  }
  ...
  addLights() {
    ...
    // ✅ 시네마 모드 제어를 위해 클래스 멤버 변수로 승격
    this.ambientLight = new THREE.AmbientLight(0xffffff, 0.1); // 0.2 → 0.1 (아주 낮춤)
    ...
    document.addEventListener('keydown', (event) => {
      switch(event.code) {
        ...
        case 'ShiftRight':
          ...
          break;
        case 'KeyC':
          // [DEBUG] 'C' 키로 시네마 모드 강제 테스트
          if (!event.repeat && this.cinemaController) {
            console.log("⌨️ [Debug] 'C' key pressed. Toggling Cinema Mode.");
            
            if (this.isCinemaMode) {
              // 이미 켜져있으면 끄기
              console.log("⌨️ [Debug] 현재 시네마 모드 활성화됨. 해제합니다.");
              this.cinemaController.exitCinema();
            } else {
              // 꺼져있으면 켜기 (TV 위치를 찾아서 전달)
              console.log("⌨️ [Debug] 시네마 모드 진입 시도. TV 그룹 검색 중...");
              ...
              this.scene.traverse((obj) => {
                if (obj.name === 'TV_Group') {
                  tvGroup = obj;
                }
              });
              
              // 방법 2: meshes 배열에서 찾기 (userData.videoElement가 있는 것)
              if (!tvGroup) {
              ...
              if (tvGroup) {
                ...
                tvGroup.getWorldPosition(tvWorldPosition);
                console.log("⌨️ [Debug] TV 그룹 발견. 위치:", tvWorldPosition);
                this.cinemaController.enterCinema(tvWorldPosition);
              } else {
                // TV를 못 찾으면 대충 현재 위치 앞 10m로 테스트
                console.warn("⌨️ [Debug] TV 그룹을 찾을 수 없습니다. 기본 위치로 테스트합니다.");
                const defaultPos = new THREE.Vector3(0, 0, -15);
                this.cinemaController.enterCinema(defaultPos);
              }
            }
          } else if (!this.cinemaController) {
            console.error("⌨️ [Debug] CinemaController가 없습니다!");
          }
          break;
      }
    });
      ...
      if (clickedObject.userData && clickedObject.userData.type === 'soundbarButton') {
        ...
        if (buttonType === 'playPause') {
          ...
          if (video.paused) {
            ...
            // ✅ 시네마 모드 진입 (TV 재생 시)
            console.log("👆 [Showroom] Play clicked. Controller exists?", !!this.cinemaController);
            if (this.cinemaController) {
              // TV 그룹의 월드 위치 계산
              const tvWorldPosition = new THREE.Vector3();
              tvGroup.getWorldPosition(tvWorldPosition);
              console.log("🎬 [Showroom] Calling enterCinema with TV position:", tvWorldPosition);
              this.cinemaController.enterCinema(tvWorldPosition);
            } else {
              console.error("❌ [Showroom] CinemaController가 없습니다!");
            }
          } else {
            video.pause();
            ...
            // ✅ 시네마 모드 해제 (TV 일시정지 시)
            console.log("⏸️ [Showroom] Pause clicked. Calling exitCinema");
            if (this.cinemaController) {
              this.cinemaController.exitCinema();
            } else {
              console.error("❌ [Showroom] CinemaController가 없습니다!");
  ...
  animate() {
    ...
    // ✅ 시네마 모드 제어권 분기점: 시네마 모드일 때는 FPS 카메라 제어 비활성화
    // (별도 모듈에서 카메라를 제어하므로 Showroom.js는 간섭하지 않음)
    if (this.isCinemaMode && this.frameCount % 60 === 0) {
      console.log("🎬 [Showroom] isCinemaMode=true, FPS 컨트롤 비활성화됨");
    }
    if (!this.isCinemaMode) {
      // [1] FPS 카메라 회전 적용
    ...
    // ✅ 시네마 모드 컨트롤러 업데이트 (카메라 및 조명 부드러운 전환)
    // [DEBUG] 주기적 연결 확인 로그 (300프레임마다, 약 5초마다)
    if (this.frameCount % 300 === 0 && this.frameCount > 0) {
      console.log("🔄 [Showroom Loop] Controller exists?", !!this.cinemaController);
      console.log("🔄 [Showroom Loop] isCinemaMode:", this.isCinemaMode);
      if (this.cinemaController) {
        console.log("🔄 [Showroom Loop] Controller isActive:", this.cinemaController.isActive);
      }
    }
    
    if (this.cinemaController) {
      this.cinemaController.update();
    } else if (this.isCinemaMode) {
      // 시네마 모드가 켜져있는데 컨트롤러가 없으면 경고
      console.warn("⚠️ [Showroom] isCinemaMode=true인데 cinemaController가 없습니다!");
    }
