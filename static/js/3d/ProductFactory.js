...
class ProductFactory {
  ...
  createPriceLabel(product, position) {
    ...
    // ✅ BoxGeometry 생성 (가로 1.5m, 세로 2.0m, 두께 0.1m) - 세로 방향으로 크기 증가
    const geometry = new THREE.BoxGeometry(1.5, 2.0, 0.1);
    ...
    // ✅ 바닥 방향으로 조금 더 이동 (Y축 -0.1, 기존 +0.1에서 추가 감소)
    // ✅ 유리 장식장(LuxeDisplay3D) 방향으로 조금 이동 (Z축 -0.5)
    menuMesh.position.set(pos.x, pos.y + labelYOffset - 0.1, pos.z + 1.5);
  ...
  /**
   * Crown3D 가구 생성 (외부 GLB 모델 로드)
   * @param {THREE.Vector3|Object} position - 위치
   * @param {Object} options - 옵션 (modelPath 등)
  ...
  createCrown3D(position, options = {}) {
    ...
    const Crown3DClass = Crown3D || window.Crown3D;
    ...
    try {
      // 모델 생성 (options에서 modelPath 전달 가능)
      const product = options.modelPath ? { userData: { modelPath: options.modelPath } } : null;
      const group = Crown3DClass.createModel(product, pos, false);
      
      if (!group) {
        console.error('      ❌ [ProductFactory] Crown3D 그룹 생성 실패!');
        return null;
      }
      
      // 바운딩 박스 확인
      if (group.userData.boundingBox) {
        const { size, center } = group.userData.boundingBox;
        console.log(`      📦 [ProductFactory] Crown3D 바운딩 박스: 크기 (${size.x.toFixed(2)}, ${size.y.toFixed(2)}, ${size.z.toFixed(2)}), 중심 (${center.x.toFixed(2)}, ${center.y.toFixed(2)}, ${center.z.toFixed(2)})`);
      }
      ...
      console.error('      ❌ [ProductFactory] Crown3D 생성 중 오류:', error);
      console.error('      ❌ [ProductFactory] 에러 스택:', error.stack);
      return null;
    }
  }
