export async function initVipNotice({
  mountSelector,
  forceParam = 'vip',
} = {}) {
  try {
    ...
    if (!mount) return;
    const forceVip =
      new URLSearchParams(location.search).get(forceParam) === '1';
    ...
    if (!forceVip && (!user || (user.plan_type || '').toLowerCase() !== 'vip'))
      return;
...
export function updateVipHeaderComparison(headers) {
  ...
  [
    ...
    cell.style.color =
      status === '✓' ? '#059669' : status === '?' ? '#d97706' : '#dc2626';
    // 애니메이션 트리거
