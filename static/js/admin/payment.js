...
function getPaymentMethodBadge(paymentMethod, pgProvider) {
  ...
  // 현금(증빙): 'trans', 'vbank' 포함
    ...
    method === 'trans' ||
    method === 'vbank' ||
    method.includes('trans') ||
    method.includes('vbank')
  ) {
...
async function loadPayments(
  ...
  try {
    ...
    tbody.innerHTML =
      '<tr><td colspan="11" class="text-center py-4 text-muted">결제 데이터를 불러오는 중...</td></tr>';
    ...
    if (result.success && result.data) {
      ...
      try {
        ...
        updateRecentPaymentsFeed(
          result.data.latest_payments || result.data.payments || []
        );
      ...
      tbody.innerHTML =
        '<tr><td colspan="11" class="text-center py-4 text-muted">결제 데이터가 없습니다.</td></tr>';
    }
  } catch (error) {
...
function renderPaymentTable(data) {
  ...
  if (payments.length === 0) {
    ...
    tbody.innerHTML =
      '<tr><td colspan="9" class="text-center py-4 text-muted">결제 데이터가 없습니다.</td></tr>';
    return;
  }
  ...
  try {
    tbody.innerHTML = payments
      .map((payment) => {
        ...
        // 유저 정보 표시 (이름과 이메일)
        const userName =
          payment.user_name || `유저 #${payment.user_id || 'N/A'}`;
...
function updateKPICards(data) {
  ...
  if (data.daily_revenue_trend) {
    console.log(
      '[updateKPICards] Using daily_revenue_trend:',
      data.daily_revenue_trend
    );
    updateRevenueChartFromTrend(data.daily_revenue_trend);
  } else {
    console.warn(
      '[updateKPICards] daily_revenue_trend not found, using fallback'
    );
...
function updateRecentPaymentsFeed(payments) {
  ...
  if (recentPayments.length === 0) {
    feed.innerHTML =
      '<div class="text-center py-4 text-muted"><small>최근 결제 내역이 없습니다.</small></div>';
    return;
  }

  feed.innerHTML = recentPayments
    .map(
      (payment) => `
...
function updateRevenueChart(payments) {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const dayRevenue = payments
      .filter(
        (p) => p.created_at.startsWith(dateStr) && p.status === 'completed'
      )
...
async function viewPaymentDetail(paymentId) {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json();
      throw new Error(
        errorData.message || `결제 정보 조회 실패: ${response.status}`
      );
    }
...
async function deletePayment(paymentId) {
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json();
      throw new Error(
        errorData.message || `결제 삭제 실패: ${response.status}`
      );
    }
...
async function cancelPayment(paymentId) {
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json();
      throw new Error(
        errorData.message || `결제 취소 실패: ${response.status}`
      );
    }
...
function handlePeriodButtonClick(period) {
  ...
  const clickedButton = document.querySelector(
    `#periodButtonGroup button[data-period="${period}"]`
  );
  if (clickedButton) {
...
function initPaymentManagement() {
  ...
  // 상태 탭 클릭 이벤트
  document
    .querySelectorAll('#paymentStatusTabs button[data-status]')
    .forEach((button) => {
      button.addEventListener('click', (e) => {
...
async function openCreatePaymentModal() {
  ...
  document.getElementById('expectedPaymentAmount').textContent =
    '상품을 선택하면 예상 결제액이 표시됩니다.';
...
async function loadProductsForPayment() {
  try {
    ...
    products.forEach((product) => {
      ...
      const mappedType =
        nameTypeMap[normalizedType] ||
        nameTypeMap[normalizedName] ||
        normalizedType;
    ...
    const displayLabel = {
      basic: (product) =>
        `[일반] 기준 단가 (1개당 ${formatCurrency(product.price || 0)})`,
      package: (product) =>
        `[할인] 프리미엄 (${formatNumber(product.token_amount || 0)}개/${formatCurrency(product.price || 0)})`,
      subscription: (product) =>
        `[구독] 무제한권 (월 ${formatCurrency(product.price || 0)})`,
      event: (product) =>
        `[이벤트] 웰컴 토큰 (${formatNumber(product.token_amount || 0)}토큰/${formatCurrency(product.price || 0)})`,
      event_period: (product) =>
        `[이벤트] 무료 체험 (${product.duration_days || 0}일/${formatCurrency(product.price || 0)})`,
    };
...
function calculateExpectedAmount() {
  const productSelect = document.getElementById('createPaymentProductId');
  const quantityInput = document.getElementById('createPaymentQuantity');
  const quantityFieldContainer = document.getElementById(
...
async function searchUsersForPayment(searchQuery) {
  try {
    ...
    const response = await fetch(
      `/admin/api/users/search?q=${encodeURIComponent(searchQuery)}&limit=10`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
...
function renderUserSearchResults(users) {
  ...
  resultsContainer.innerHTML = users
    .map((user) => {
      const planBadge =
        user.plan_type === 'gold-vip'
          ? '<span class="badge bg-warning text-dark ms-2">Gold</span>'
          : user.plan_type === 'premium-vip'
            ? '<span class="badge bg-info ms-2">Premium</span>'
            : user.plan_type === 'vip'
...
function selectUserForPayment(userId, username, email) {
  console.log('[selectUserForPayment] 사용자 선택:', {
    userId,
    username,
    email,
  });
...
async function submitCreatePayment() {
  ...
  const submitButton = document.querySelector(
    '#createPaymentModal .modal-footer button.btn-primary'
  );
  ...
  try {
    ...
    const productId = parseInt(
      document.getElementById('createPaymentProductId').value
    );
    const quantity =
      parseInt(document.getElementById('createPaymentQuantity').value) || 1;
