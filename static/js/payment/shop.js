...
(function () {
  ...
  function openCheckoutModal(btn) {
    ...
    currentProduct.duration =
      parseInt(btn.getAttribute('data-duration'), 10) || 0;
    ...
    const taxEvidenceCheckbox = document.getElementById(
      'modalTaxEvidenceCheckbox'
    );
  ...
  async function confirmPurchase() {
    ...
    // Input 필드에서 수량 가져오기
    const quantityInput = document.getElementById('modalQuantityInput');
    const isQuantityInputVisible =
      quantityInput && !quantityInput.closest('.hidden');
  ...
  function initPurchaseButtons() {
    document.addEventListener('click', function (e) {
      ...
      if (
        e.target &&
        (e.target.classList.contains('btn-purchase') ||
          e.target.classList.contains('btn-purchase-fullscreen'))
      ) {
        e.preventDefault();
  ...
  /**
   * 3D 이벤트 상품 씬 초기화 (제거됨 - 2D 디자인으로 전환)
   */
  function init3DGiftBoxes() {
    // 3D 기능 제거됨 - 더 이상 사용하지 않음
    return;
  }
