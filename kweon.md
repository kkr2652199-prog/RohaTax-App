# [2026-01-05 10:26:25 KST] - 모바일 이미지 확대/축소 기능 개선 및 공간 효율화
- 커밋 해시: 466709cc88ad458b29071be6d8776307575af016
- **작업 상세 내역**:
  1. **핀치 줌 기능 추가**: 자연스러운 1~5배 확대/축소 지원 (고정값 2.5배 제거)
  2. **각 이미지 독립 구간 구분**: 스크롤 스냅 강화 (`scroll-snap-stop: always`)로 중간 영역 미노출, 1번 클릭 후 8번 클릭 시 즉시 8번으로 고정
  3. **여백 클릭 방지**: `slide-wrapper`, `slide`에 `pointer-events: none` 적용, 이미지만 `pointer-events: auto`로 터치 이벤트 최적화
  4. **공간 효율화**: 
     - 위아래 여백 컨테이너 제거 (`hometax-guide-slider-container` padding: 0)
     - 이미지 영역 확대 (52vh → 70vh)
     - 가이드 탭 간격/패딩 축소 (gap: 8px → 6px, padding: 10px 6px → 8px 4px)
     - 폰트 크기 축소 (11px → 10px)
  5. **확대된 이미지 팬 기능 개선**: 현재 확대 비율에 맞춰 이동 범위 계산, 모든 영역 탐색 가능
  6. **스크롤 정확도 개선**: `scrollToSlide`에서 즉시 정확한 위치로 이동, `onScroll`에서 2px 이상 오차 시 자동 조정

# [2026-01-03 13:19:17 KST] - 스마트폰 메인 첫 화면(히어로) 이미지 대폭 확대 및 UX 정리
- 커밋 해시: 8dbe240
- **작업 상세 내역**:
  1. **히어로 이미지 대폭 확대(모바일 전용)**: 2컬럼 비율을 이미지 쪽으로 확장하고 이미지 최대 높이를 상향하여 PC처럼 “이미지/텍스트 밸런스”를 맞췄습니다. (PC/태블릿 영향 0)
  2. **이미지 스와이프 레일 개선**: 카드 폭을 확대하고(94%) 내부 패딩을 줄여(4px) 이미지가 “확실히 크게” 보이도록 했습니다.
  3. **개발 반영/캐시 옵션 정리**: `DEV_CACHE_BUST=1`일 때만 정적 리소스 버전이 변하도록 하여, 기본은 캐시 유지(체감 속도 우선)로 정리했습니다.
  4. **요청 제한(429) 회피(개발 환경)**: 개발 환경에서 Flask-Limiter를 비활성화하여 스마트폰 반복 테스트 시 접속 차단 문제를 방지했습니다.
  5. **스마트폰 접속 편의 스크립트 추가**: `setup_mobile_access.bat`로 IP 안내 및 방화벽 허용(5000) 절차를 자동화했습니다.

# [2026-01-02 17:27:00 KST] - 스마트폰 아이디 탭 모달 중앙 정렬 및 크기 축소
- 커밋 해시: e1f9b56
- **작업 상세 내역**:
  1. **모달 중앙 정렬**: 아이디 클릭 시 나타나는 바로가기 모달을 스마트폰 화면 중앙에 배치하도록 `position: fixed`, `left: 50%`, `transform: translateX(-50%)` 적용.
  2. **크기 대폭 축소**: 모달 너비 280px, 아이템 높이 50px, 아이콘 20px, 텍스트 10px로 축소하여 시각적 간섭 최소화.
  3. **햄버거 모달과 겹침 방지**: `top: 98%`로 설정하여 햄버거 모달(사이드 메뉴)과 완전히 분리되도록 위치 조정.
  4. **PC/태블릿 보호**: 모든 수정사항을 `@media (max-width: 480px)` 내부에만 적용하여 데스크탑 화면에는 영향 없음.

# [2026-01-02 12:15:00 KST] - 홈택스 가이드 슬라이더(How it Works) 버튼 작동 오류 해결
- **작업 상세 내역**:
  1. **슬라이더 스크립트 전면 개편**: `homepage.js`에서 기존의 잘못된 선택자(`.hometax-guide-slider`, `.guide-slide` 등)를 현재 HTML 구조(`.tab-item`, `.slide` 등)에 맞게 전면 수정했습니다.
  2. **탭 버튼 클릭 이벤트 바인딩**: 8개의 단계별 탭 버튼을 클릭했을 때 해당 슬라이드로 즉시 이동하도록 로직을 추가했습니다.
  3. **컨트롤 버튼 동기화**: 이전/다음/재생 버튼이 현재 슬라이드 상태와 완벽히 연동되도록 수정했습니다.
  4. **마우스 클릭 효과 타이밍 조정**: 슬라이드 전환 시 마우스 클릭 애니메이션이 자연스럽게 나타나도록 지연 시간을 최적화했습니다.

# [2026-01-02 12:00:00 KST] - 헤더 모바일 메뉴 겹침 현상 및 UI 레이아웃 최적화
- **작업 상세 내역**:
  1. **데스크탑 모바일 메뉴 숨김 처리**: 데스크탑 해상도에서 모바일 전용 사이드 메뉴(`mobile-side-menu`)가 불필요하게 노출되어 헤더 아래에 겹치던 현상을 `display: none`으로 해결했습니다.
  2. **햄버거 버튼 노출 방지**: 스마트폰 전용 메뉴 버튼이 데스크탑에서도 보이던 문제를 수정했습니다.
  3. **헤더 반응형 레이아웃 개선**: 메뉴 간격을 조정하여 중간 해상도(1200px~1400px)에서도 텍스트 겹침이 발생하지 않도록 CSS를 보강했습니다.

# [2026-01-02 11:30:00 KST] - 배포 서버 정밀 성능 최적화 및 숨겨진 렉 요소 제거
- **작업 상세 내역**:
  1. **AI 블로그 스튜디오 캐싱 극대화**: `kweon21_routes.py`에서 빌드 자산(JS/CSS)에 영구 캐싱(`immutable`)을 적용하여 재접속 시 로딩 속도를 대폭 향상했습니다.
  2. **병렬 로딩 최적화**: 모바일 전용 CSS 로딩 방식을 `@import`(순차)에서 `<link>`(병렬)로 변경하여 초기 렌더링 지연을 해소했습니다.
  3. **중복 리소스 로드 제거**: 비디오 플레이어 스크립트가 중복 로드되던 문제를 해결하고 불필요한 콘솔 로그를 정리했습니다.
  4. **외부 이미지 최적화**: Unsplash 고해상도 이미지의 해상도 파라미터를 조정하여 시각적 품질은 유지하되 데이터 전송량을 줄였습니다.
