# [2026-01-04 13:48:00 KST] - 스마트폰 통계/2번 화면(Why RohaTax) 여백 최단화 및 겹침 방지
- 커밋 해시: 0b77f5d
- **작업 상세 내역**:
  1. **Stats(통계) 섹션 여백 최단화(모바일 전용)**: `stats-section` 상/하 여백을 0에 가깝게 줄이고, 카드(4개) 간격/폭/내부 패딩을 더 컴팩트하게 조정했습니다.
  2. **Why RohaTax 제목/부제목 하단 이동(모바일 전용)**: `stats-section`과 겹치지 않도록 섹션 시작/헤더 블록의 간격을 조정해 제목/부제목을 대폭 아래로 내렸습니다.
  3. **모바일 히어로/타겟 카드 UX 유지 보강**: `03_hero.css` 및 `homepage.js`에서 모바일 전용 레이아웃/토글/레일 동작을 유지하면서 PC/태블릿 영향 없이 정리했습니다.
  4. **모바일 테스트 유틸 정리**: `start_ngrok_tunnel.bat`을 정리해 스마트폰 테스트 플로우를 유지했습니다.
