...
def enhance_first_priority_with_second_priority_logic(
    ...
    try:
        ...
        for recipient in first_priority_recipients:
            ...
            if matched_row is not None and (not enhanced_recipient.get('사업자등록번호') or enhanced_recipient.get('사업자등록번호').strip() == ''):
                ...
                for col_name in ['사업자번호', '사업자등록번호', '사업자번호']:
                    if col_name in df.columns:
                        business_number_raw = str(matched_row.get(col_name, '')).strip()
                ...
                if business_number_raw:
                    ...
                    if business_number and is_valid_business_number(business_number):
                        ...
                        log.warning(f"⚠️ 사업자등록번호 재추출 실패: 상호명 '{enhanced_recipient.get('상호')}' 유효하지 않은 사업자등록번호")
                else:
                    log.warning(f"⚠️ 사업자등록번호 재추출 실패: 상호명 '{enhanced_recipient.get('상호')}' 매칭 실패")
            elif matched_row is None and (not enhanced_recipient.get('사업자등록번호') or enhanced_recipient.get('사업자등록번호').strip() == ''):
                log.warning(f"⚠️ 사업자등록번호 재추출 실패: 상호명 '{enhanced_recipient.get('상호')}' 매칭 실패")
            
            # 이메일이 없는 경우 2순위 방식으로 재추출
            if matched_row is not None and (not enhanced_recipient.get('사업자이메일') or enhanced_recipient.get('사업자이메일').strip() == ''):
                log.info("🔍 5형제 중 사업자이메일 누락 - 2순위 방식으로 재추출")
                
                # 2순위 방식으로 이메일 추출 (다양한 컬럼명 지원)
                email_raw = None
                for col_name in ['사업자이메일', '이메일', '이메일주소', 'email']:
                    if col_name in df.columns:
                        email_raw = str(matched_row.get(col_name, '')).strip()
                        if email_raw and email_raw != 'nan':
                            break
                
                if email_raw:
                    if is_valid_email(email_raw):
                        email = field_extractors.extract_email(email_raw, matched_row)
                        if email:
                            enhanced_recipient['사업자이메일'] = email
                            log.info(f"✅ 특별대우 이메일 추출: '{email_raw}' → '{email}'")
                        else:
                            log.warning(f"⚠️ 이메일 재추출 실패: 상호명 '{enhanced_recipient.get('상호')}' 이메일 추출 실패")
                    else:
                        log.warning(f"⚠️ 이메일 재추출 실패: 상호명 '{enhanced_recipient.get('상호')}' 유효하지 않은 이메일")
                else:
                    log.warning(f"⚠️ 이메일 재추출 실패: 상호명 '{enhanced_recipient.get('상호')}' 매칭 실패")
            
            # 주소가 없는 경우 2순위 방식으로 재추출
            if matched_row is not None and (not enhanced_recipient.get('사업장주소') or enhanced_recipient.get('사업장주소').strip() == ''):
                log.info("🔍 5형제 중 사업장주소 누락 - 2순위 방식으로 재추출")
                
                # 2순위 방식으로 주소 추출 (다양한 컬럼명 지원)
                address_raw = None
                for col_name in ['도착지주소', '주소', '사업장주소', '배달주소']:
                    if col_name in df.columns:
                        address_raw = str(matched_row.get(col_name, '')).strip()
                        if address_raw and address_raw != 'nan':
                            break
                
                if address_raw:
                    if is_valid_address(address_raw):
                        address = field_extractors.extract_address(address_raw, matched_row)
                        if address:
                            enhanced_recipient['사업장주소'] = address
                            log.info(f"✅ 특별대우 주소 추출: '{address_raw}' → '{address}'")
                        else:
                            log.warning(f"⚠️ 주소 재추출 실패: 상호명 '{enhanced_recipient.get('상호')}' 주소 추출 실패")
                    else:
                        log.warning(f"⚠️ 주소 재추출 실패: 상호명 '{enhanced_recipient.get('상호')}' 유효하지 않은 주소")
                else:
                    log.warning(f"⚠️ 주소 재추출 실패: 상호명 '{enhanced_recipient.get('상호')}' 매칭 실패")
            
            # 대표자명이 없는 경우 2순위 방식으로 재추출
            if matched_row is not None and (not enhanced_recipient.get('대표명') or enhanced_recipient.get('대표명').strip() == ''):
                log.info("🔍 5형제 중 대표명 누락 - 2순위 방식으로 재추출")
                
                # 2순위 방식으로 대표자명 추출 (우선순위 키워드 활용)
                representative_candidate = extract_representative_simple(matched_row, column_names)
                if representative_candidate and is_valid_representative_name(representative_candidate):
                    ...
                    representative_raw = str(matched_row.get('등록자명', '')).strip()
                    if representative_raw and representative_raw.lower() not in {'nan', 'none', 'null', ''}:
                        if is_valid_representative_name(representative_raw):
                            representative = field_extractors.extract_representative(representative_raw, matched_row)
                            if representative:
                                enhanced_recipient['대표명'] = representative
                                log.info(f"✅ 특별대우 대표자명(등록자명 활용) 추출: '{representative_raw}' → '{representative}'")
                            else:
                                log.warning(f"⚠️ 대표자명 재추출 실패: 상호명 '{enhanced_recipient.get('상호')}' 대표자명 추출 실패")
                        else:
                            log.warning(f"⚠️ 대표자명 재추출 실패: 상호명 '{enhanced_recipient.get('상호')}' 유효하지 않은 대표자명")
                    else:
