...
(function() {
    ...
    if (!canvas) {
        console.warn('Snowflake canvas not found');
        return;
    }

    const gl = canvas.getContext('webgl2');
    if (!gl) {
        console.warn('WebGL 2 not supported, snowflake background disabled');
        canvas.style.display = 'none';
        return;
    }
    
    console.log('Snowflake background initialized');
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // 배율 조절: uZoom uniform 사용 (기본값 2.5에 배율 적용)
    float baseZoom = 2.5 * uZoom;
    zoom = baseZoom; // 전역 zoom 변수 초기화
    float initialZoom = zoom; // 초기 zoom 값 저장
    ...
    for (int i=0; i<layersblob;i++) {
        ...
        // 배율 조절 시 레이어별 zoom 감소도 비례적으로 조절
        zoom = initialZoom - (mzd * float(i+1));
    }
...
void main() {
    ...
    // 배율 및 투명도 설정 (기본값)
    let currentZoom = 1.0; // 기본 배율
    let currentOpacity = 0.3; // 기본 투명도 (배경이므로 약하게)

    // 설정값을 로컬 스토리지에서 불러오기
    const savedZoom = localStorage.getItem('snowflakeZoom');
    const savedOpacity = localStorage.getItem('snowflakeOpacity');
    if (savedZoom) currentZoom = parseFloat(savedZoom);
    if (savedOpacity) currentOpacity = parseFloat(savedOpacity);
    ...
    function render(time) {
        ...
        gl.uniform1f(zoomUniformLocation, currentZoom);
        gl.uniform1f(opacityUniformLocation, currentOpacity);
    ...
    // 배율 조절 함수 (외부에서 호출 가능)
    window.snowflakeBackground = {
        setZoom: function(zoom) {
            currentZoom = Math.max(0.1, Math.min(3.0, zoom));
            localStorage.setItem('snowflakeZoom', currentZoom);
        },
        setOpacity: function(opacity) {
            currentOpacity = Math.max(0.0, Math.min(1.0, opacity));
            localStorage.setItem('snowflakeOpacity', currentOpacity);
        },
        getZoom: function() {
            return currentZoom;
        },
