...
def kweon21_index(path):
    ...
    if os.path.exists(index_path):
        ...
        # index.css 파일이 실제로 존재하는 경우에만 변환
        index_css_path = os.path.join(dist_dir, "index.css")
        if os.path.exists(index_css_path):
            html_content = re.sub(
                r'href="/index\.css', 'href="/studio/index.css', html_content
            )
        else:
            # index.css가 없으면 해당 링크 제거
            html_content = re.sub(
                r'<link[^>]*href="/index\.css"[^>]*>', "", html_content
            )
