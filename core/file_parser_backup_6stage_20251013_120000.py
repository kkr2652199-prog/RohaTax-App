...
class FileParser:
    ...
    def parse_file(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"파일 파싱 오류: {str(e)}")
            return self._create_error_response(f"파일 파싱 중 오류 발생: {str(e)}")
    ...
    def _inspect_all_sheets(self, workbook) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.warning(f"업종별 설정 로딩 실패, 기본값 사용: {str(e)}")
            weights = {
                'business_number': 30,
                'representative': 10,
                'address': 30,
    ...
    def _parse_excel(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"Excel 파싱 오류: {str(e)}")
            return self._create_error_response(f"Excel 파일 파싱 오류: {str(e)}")
    ...
    def _parse_csv(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"CSV 파싱 오류: {str(e)}")
            return self._create_error_response(f"CSV 파일 파싱 오류: {str(e)}")
